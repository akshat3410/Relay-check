...
export const missingReadmeRule: Rule = {
  ...
  rationale:
    'The README is the primary entry point for developers and users. It should explain what the project is, how to install dependencies, and how to run the project.',
  ...
  execute(ctx: ProjectContext): Finding[] {
    ...
    if (!readmeFile) {
      findings.push({
        ...
        suggestion:
          'Create a README.md file in the root of the project to document install, build, and usage instructions.',
        docs: 'https://relay.dev/rules/DOC-001',
      });
    }
...
export const missingLicenseRule: Rule = {
  ...
  rationale:
    'Without an explicit license, a repository falls under default copyright laws, meaning no one can legally copy, distribute, or modify the code.',
  ...
  execute(ctx: ProjectContext): Finding[] {
    ...
    if (!hasLicense) {
      findings.push({
        ...
        suggestion:
          'Add an open source LICENSE file (e.g. MIT, Apache-2.0, GPL-3.0) to your repository root.',
        docs: 'https://relay.dev/rules/DOC-002',
      });
    }

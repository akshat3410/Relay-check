...
export const missingDockerHealthcheckRule: Rule = {
  ...
  rationale:
    'Without a HEALTHCHECK, orchestrators like Kubernetes or Docker Swarm only know if the container process is running, not if the application is actually healthy and responsive.',
    ...
    for (const file of ctx.sourceFiles) {
      ...
      if (!hasHealthcheck) {
        findings.push({
          ...
          suggestion:
            'Add a HEALTHCHECK instruction to the Dockerfile, e.g.: HEALTHCHECK --interval=30s --timeout=30s --start-period=5s --retries=3 CMD curl -f http://localhost:3000/api/health || exit 1',
          docs: 'https://relay.dev/rules/DEPLOY-001',
        });
      }
    }
...
export const dockerRunAsRootRule: Rule = {
  ...
  rationale:
    'Running containers as root violates the principle of least privilege. If a container is compromised, the attacker has root privileges on the host kernel/container namespace.',
    ...
    for (const file of ctx.sourceFiles) {
      ...
      if (userMatches.length === 0) {
        findings.push({
          ...
          file: file.relativePath,
          suggestion:
            'Create a non-root user in the Dockerfile and set it using the USER directive: e.g. USER node or USER appuser.',
          docs: 'https://relay.dev/rules/DEPLOY-002',
        });
      } else {

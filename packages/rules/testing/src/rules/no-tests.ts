...
export const noTestsRule: Rule = {
  ...
  rationale: 'Shipping software without automated testing increases the risk of regressions, deployment failures, and production bugs.',
  ...
  execute(ctx: ProjectContext): Finding[] {
    ...
    if (!hasTestFiles && !hasTestDeps) {
      findings.push({
        ...
        suggestion: 'Install a test runner like Vitest (pnpm add -D vitest) or Jest, and write unit/integration tests for core business logic.',
        docs: 'https://relay.dev/rules/TEST-002',
      });
      ...
      findings.push({
        ...
        severity: 'medium',
        category: 'testing',
        message: 'Testing frameworks are installed but no test files (*.test.ts, *.spec.ts) were found',

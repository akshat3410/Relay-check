...
export const emptyInteractiveRule: Rule = {
  ...
  rationale: 'Interactive elements like buttons and links must have a name to be announced by screen readers. Empty buttons or links offer no context to users navigating with assistive technologies.',
  ...
  execute(ctx: ProjectContext): Finding[] {
    ...
    for (const file of ctx.sourceFiles) {
      ...
      while ((btnMatch = buttonPattern.exec(file.content)) !== null) {
        ...
        const hasAriaLabel = /\baria-label\s*=/i.test(tagAttribs) || /:\baria-label\s*=/i.test(tagAttribs);
        const hasAriaLabelledBy = /\baria-labelledby\s*=/i.test(tagAttribs) || /:\baria-labelledby\s*=/i.test(tagAttribs);
        ...
        if (!hasContent && !hasAriaLabel && !hasAriaLabelledBy) {
          ...
          findings.push({
            ...
            suggestion: 'Add visible text content inside the button, or add an aria-label attribute.',
            docs: 'https://relay.dev/rules/A11Y-004',
          });
        }
      }
      ...
      let lnkMatch: RegExpExecArray | null;
      while ((lnkMatch = linkPattern.exec(file.content)) !== null) {

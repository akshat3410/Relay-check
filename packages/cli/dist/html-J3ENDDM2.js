...
// ../reporters/dist/chunk-EISBNDN6.js
var STATUS_ICONS = {
  ship: "\u2705",
  warn: "\u26A0\uFE0F",
  hold: "\u274C",
...
var HtmlReporter = class {
  ...
  render(result) {
    ...
    const categoriesHtml = result.categoryScores.map((cat) => {
    ...
    const findingsHtml = result.findings.map((finding) => {
      ...
      return `
          <div class="p-5 border border-gray-200 rounded-xl hover:border-gray-300 transition-all bg-white shadow-sm flex flex-col gap-2">
            ...
            ${loc ? `<div class="text-sm text-gray-500 font-mono mt-1">\u{1F4CD} ${loc}</div>` : ""}
            ...
            ${finding.suggestion ? `<div class="text-sm text-emerald-700 font-medium mt-1">\u{1F4A1} Suggestion: ${finding.suggestion}</div>` : ""}
          </div>
        `;
    }).join("");
    return `
  ...
  <div class="max-w-4xl mx-auto px-4 pt-12">
    ...
    <div class="mt-8">
      ...
      <div class="flex flex-col gap-4">
        ${findingsHtml || '<div class="bg-white border border-gray-200 rounded-2xl p-8 text-center text-gray-500 shadow-sm">\u{1F389} No issues found! Your project is clean and ready.</div>'}
      </div>
    </div>
  </div>
</body>
...
//# sourceMappingURL=html-J3ENDDM2.js.map